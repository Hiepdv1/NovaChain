...
const TransactionCard = ({ transaction }: TransactionCardProps) => {
  ...
  const onRefBlock = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    e.stopPropagation();
    router.push(`/blocks/${transaction.BID}`);
  };

  return (
    <Link className="block" href={`/txs/${transaction.TxID}`}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-xl hover:scale-[1.02] hover:border-blue-300 dark:hover:border-blue-600 transition-all duration-300 cursor-pointer group">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <button
            onClick={onRefBlock}
            className="cursor-pointer relative z-50 bg-gray-50 rel dark:bg-gray-900 rounded-xl p-3 border border-gray-100 dark:border-gray-700"
          >
            <div className="text-xs text-gray-500 dark:text-gray-400 mb-1 font-medium flex items-center gap-1">
              <Layers className="w-3 h-3" />
              Block
            </div>
            <div className="text-base font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex items-center gap-1 group-hover:underline">
